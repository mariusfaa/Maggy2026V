...
%   .skipped         logical, true if save file already existed (resume)
    ...
    % --- Defaults ---
    % Default plant_n to controller_n so plant and controller use the same
    % discretisation — matches the validated workingSimulator.m setup. The
    % decoupled n=30 plant introduced model mismatch that the 12-state OCP
    % could not compensate (see plan doc, Phase B.1).
    if ~isfield(cfg, 'plant_n') || isempty(cfg.plant_n);  cfg.plant_n  = cfg.controller_n; end
    ...
    % --- Build OCP solver FIRST, then plant (timed) ---
    % Order matters: AcadosOcpSolver's post-build cleanup ("delete template...")
    % invalidates the AcadosSimSolver's MATLAB class file if the sim was built
    % first. Mirrors the working order in workingSimulatorReducedOrderTiming.m.
    t_compile = tic;
