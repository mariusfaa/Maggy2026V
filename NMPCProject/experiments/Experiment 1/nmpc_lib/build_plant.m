...
%   plant_solver  AcadosSimSolver (12-state, IRK 4 stages / 10 steps)
    ...
    % Put plant code in its own dir so AcadosOcpSolver's post-build
    % cleanup of c_generated_code/ doesn't invalidate the plant's
    % MATLAB class file. Without this, the *second* OCP build in a
    % session breaks the cached plant_solver with:
    %   "Method 'set' is not defined for class 'plant_..._mex_sim_solver'
    %    or is removed from MATLAB's search path."
    plant_dir = fullfile(pwd, 'c_generated_code_plant');
