...
% c_generated_code/<name>/ and CMake nests it inside paths like
% CMakeFiles/model_<name>.dir/<hash32>/<name>_impl_dae_fun.c.obj.d,
% so on Windows (260-char path limit) we must keep the name SHORT.
%
% Format:  m_<stage>_<modelchar>_<hash8>     (e.g. "m_1b_r_AABBCCDD")
% Length:  10..14 chars depending on stage. Hash is over the full
%          descriptor so distinct cfgs get distinct names.
    desc = sprintf('%s|%s|%s|n%d|N%d|T%d|%s|%d|%d|%s|%s|%s', ...
        cfg.stage, cfg.exp_id, M.model_kind, M.controller_n, ...
        cfg.N, round(cfg.Tf*1000), lower(cfg.integ_type), ...
        cfg.n_stages, cfg.n_steps, lower(cfg.nlp_solver), ...
        lower(cfg.qp_solver), lower(cfg.hessian_approx));
    h = dec2hex(string2hash(desc), 8);                 % 8-char hex
    if length(h) > 8; h = h(end-7:end); end             % cap at 8
    mc = lower(M.model_kind(1));                         % 'r' or 'f'
    stage_clean = regexprep(cfg.stage, '[^A-Za-z0-9]', '');
    name = sprintf('m_%s_%s_%s', stage_clean, mc, h);
    name = sanitize(name);
end

function s = sanitize(s)
    s = regexprep(s, '[^A-Za-z0-9_]', '_');
    s = regexprep(s, '_+', '_');                  % collapse consecutive _
    s = regexprep(s, '_+$', '');                  % strip trailing _
end

function h = string2hash(s)
    % Java's String.hashCode() — built into MATLAB, deterministic, no
    % overflow/saturation issues. Returns int32 (signed); reinterpret as
    % uint32 so dec2hex always produces 8 unsigned hex digits.
    h_signed = int32(java.lang.String(s).hashCode());
    h = double(typecast(h_signed, 'uint32'));
end
