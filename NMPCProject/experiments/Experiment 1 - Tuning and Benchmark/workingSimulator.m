...
%% --- BUILD OCP SOLVER ---
if ~exist('ocp_solver_full12', 'var') || ~isvalid(ocp_solver_full12)
    fprintf('\n--- Building acados OCP solver ---\n');
    ocp_solver_full12 = AcadosOcpSolver(ocp);
else
    fprintf('\n--- Reusing OCP solver ---\n');
end

%% --- BUILD SIM SOLVER ---
if ~exist('sim_solver_full12', 'var') || ~isvalid(sim_solver_full12)
    ...
    sim_solver_full12 = AcadosSimSolver(sim);
else
    fprintf('\n--- Reusing sim solver ---\n');
end
...
for k = 0:N
    alpha_k = k / N;
    ocp_solver_full12.set('x', (1 - alpha_k) * x_current + alpha_k * xEq, k);
end
for k = 0:N-1
    ocp_solver_full12.set('u', uEq, k);
end
...
for i = 1:sim_steps
    % --- Solve OCP ---
    ocp_solver_full12.set('constr_x0', x_current);
    ocp_solver_full12.solve();

    status    = ocp_solver_full12.get('status');
    sqp_iter  = ocp_solver_full12.get('sqp_iter');
    time_tot  = ocp_solver_full12.get('time_tot');
    u_applied = ocp_solver_full12.get('u', 0);
    u_applied = max(min(u_applied, 1), -1);

    % --- Simulate plant ---
    sim_solver_full12.set('x', x_current);
    sim_solver_full12.set('u', u_applied);
    sim_solver_full12.solve();
    x_next = sim_solver_full12.get('xn');

    % --- Warm-start shift ---
    x_traj = ocp_solver_full12.get('x');
    u_traj = ocp_solver_full12.get('u');
    x_traj_shift = [x_traj(:, 2:end), xEq];
    u_traj_shift = [u_traj(:, 2:end), uEq];
    for k = 0:N
        ocp_solver_full12.set('x', x_traj_shift(:, k+1), k);
    end
    for k = 0:N-1
        ocp_solver_full12.set('u', u_traj_shift(:, k+1), k);
    end
