...
% (alpha_R varies, alpha_we=10 anchor). Pass 2 runs after pass 1 picks alpha_R.
    out = {};
    for k = 1:length(cfgs)
        nm = cfgs{k}.exp_id;
        if startsWith(nm, 'aR') && contains(nm, '_we10') && ~startsWith(nm, 'aR_locked')
            % Pass 1 (varying alpha_R, fixed alpha_we=10)
...
% Pass 2: pick alpha_we minimizing J among the locked-aR runs.
    ...
    if isempty(files)
        % Defaults match the workingSimulator.m baseline.
        picks = struct('alpha_R',1.0,'alpha_we',10);
        return;
    end

    % Pass-1 anchor is alpha_we=10 (was 25 before B.4); pattern '_we10'.
    bestR.J = inf; bestR.aR = 1.0;
    bestWe.J = inf; bestWe.aWe = 10;
    for k = 1:numel(files)
        nm = files(k).exp_id;
        if summaries{k}.diverged; continue; end
        J = summaries{k}.J_integrated;
        ...
        elseif startsWith(nm, 'aR') && contains(nm, '_we10')
            tok = regexp(nm, 'aR(\w+?)_we10', 'tokens', 'once');
            if isempty(tok); continue; end
...
% lex-min on (J, t_mean) when fewer than K Pareto-optimal points exist.
    ...
    if ~any(valid)
        warning(['pick_top_1b: no Stage-1b cfgs converged without divergence ' ...
                 'for model %s. Stage 2/3 will have no top configs to expand.'], ...
                 model_kind);
        return;
    end

    pareto_idx = pareto_front(t, J, valid);
    Jv = J(valid); tv = t(valid);
    score = (J - min(Jv)) / max(eps, max(Jv) - min(Jv)) ...
          + (t - min(tv)) / max(eps, max(tv) - min(tv));
    score(~valid) = inf;
    [~, sorted] = sort(score);
