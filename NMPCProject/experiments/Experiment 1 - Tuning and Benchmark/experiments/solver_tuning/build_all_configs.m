...
function c = base_cfg(model_kind)
    ...
    c.controller_n   = 10;
    c.plant_n        = 10;   % match controller_n (was 30; see plan doc Phase B.1)
    c.N              = 20;
    c.Tf             = 0.2;
...
function cfgs = stage_1a(model_kind, ctx) %#ok<INUSD>
    base = base_cfg(model_kind);
    base.stage = '1a';
    base.exp_id = 'seed';
    base.integ_type = 'IRK';
    % Use the workingSimulator.m known-safe integrator setting (IRK 4/10)
    % for the seed run — this stage measures J_legacy for normalisation,
    % not integrator accuracy.
    base.n_stages   = 4;
    base.n_steps    = 10;
    cfgs = {base};
end
...
function cfgs = stage_0(model_kind, ctx) %#ok<INUSD>
    cfgs = {};
    base = base_cfg(model_kind);
    base.stage = '0';
    base.integ_type = 'IRK';
    % Use the workingSimulator.m known-safe integrator (IRK 4/10) for
    % weight-tuning. Stage 0 is about finding good Q/R/alpha_we, not
    % testing integrator accuracy. Using ctx.nstep_irk (picked by P0)
    % risks inherited inaccuracy causing all Stage 0 runs to diverge.
    base.n_stages   = 4;
    base.n_steps    = 10;
    ...
    % Pass 1: vary alpha_R, fix alpha_we at the workingSimulator anchor (10).
        ...
        for ii = 1:length(integ_list)
            for inlp = 1:length(nlp_list)
                for ih = 1:length(hess_list)
                    ...
                    % Clamp IRK n_steps to >= 10 so P0's pick (which may
                    % have favoured cheaper integrators against a possibly
                    % divergent reference) can't drag stability margin
                    % below the workingSimulator baseline (IRK 4/10).
                    nstep_irk_clamped = max(ctx.nstep_irk, 10);
                    nstep_erk_clamped = max(ctx.nstep_erk, 10);
                    if strcmp(intg, 'ERK')
                        c.n_stages = 4;
                        c.n_steps  = nstep_erk_clamped;
                    elseif strcmp(intg, 'IRK')
                        c.n_stages = ctx.ns_irk;
                        c.n_steps  = nstep_irk_clamped;
                    else  % GNSF
                        c.n_stages = ctx.ns_irk;
                        c.n_steps  = nstep_irk_clamped;
                    end
        ...
        for iN = 1:length(N_grid)
            for iT = 1:length(Tf_grid)
                ...
                for iN2 = 1:length(n_grid)
                    ...
                    c.plant_n       = n_grid(iN2);   % keep plant = controller (B.1)
                    cfgs{end+1} = c; %#ok<AGROW>
                end
            end
        end
