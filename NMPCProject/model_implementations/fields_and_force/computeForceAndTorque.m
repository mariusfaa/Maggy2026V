function [fx,fy,fz,tx,ty,tz] = computeForceAndTorque(x,u,params,modelId)
...
% the number of solenoids in params). modelId is either 'fast', 
...
switch modelId
    ...
    otherwise % Default is 'fast'
        ...
        % Compute force
        tangent0 = [cos(theta+pi/2); sin(theta+pi/2); zeros(size(theta))];
        tangent = R*tangent0;
        F = cross(K*params.magnet.l*tangent,[bx;by;bz]);
        
        fx = params.magnet.r*trapz_casadi([theta,2*pi],[F(1,:),F(1,1)]);
        fy = params.magnet.r*trapz_casadi([theta,2*pi],[F(2,:),F(2,1)]);
        fz = params.magnet.r*trapz_casadi([theta,2*pi],[F(3,:),F(3,1)]);
        
        % Compute torque
        nvec0 = [zeros(2,params.magnet.n); ones(1,params.magnet.n)];
        nvec = R*nvec0;
        T = cross(K*params.magnet.l*nvec,[bx;by;bz]);
        
        tx = params.magnet.r*trapz_casadi([theta,2*pi],[T(1,:),T(1,1)]);
        ty = params.magnet.r*trapz_casadi([theta,2*pi],[T(2,:),T(2,1)]);
        tz = params.magnet.r*trapz_casadi([theta,2*pi],[T(3,:),T(3,1)]);
end
