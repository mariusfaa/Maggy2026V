function runExperimentB(exp_id, integ_type, n_stages, n_steps)
% RUNEXPERIMENTB  One sub-experiment of Experiment B (SQP_RTI).
%   matlab -batch "runExperimentB('B4','ERK',4,5)"

if ischar(n_stages); n_stages=str2double(n_stages); end
if ischar(n_steps);  n_steps =str2double(n_steps);  end

exp_name=sprintf('B_%s',exp_id);
exp_desc=sprintf('SQP_RTI %s stages=%d steps=%d',integ_type,n_stages,n_steps);
fprintf('=== Experiment B — %s ===\n  %s\n\n',exp_id,exp_desc);
...
nx=10; nu=4; N=20; Tf=0.2; dt_mpc=Tf/N;
...
%% === Plant simulator (IRK 4/10, ground truth) ===
plant_solver = build_plant(sprintf('mplant_%s',exp_id), x_r, u_sym, xdot_r, f_expl_r, dt_mpc);

%% === OCP solver (SQP_RTI) ===
ocp_solver = build_ocp(sprintf('mlev_%s',exp_id), x_r, u_sym, xdot_r, f_expl_r, ...
    integ_type, n_stages, n_steps, N, Tf, Q, R, n_sbx, xEq, uEq, lbx_c, ubx_c);
...
[usab, settling, roa_max] = compute_usability( ...
    PX, PU, Perr, Ppos, Pang, Pslack, xEq, uEq, dt_mpc, n_actual, ...
    lbx_c, ubx_c, idx_c, pert_scale, dx_dir, ...
    ocp_solver, plant_solver, N, sim_steps, diverged);
...
%% === Save ===
sd = build_save_struct(exp_name, exp_desc, 'SQP_RTI', integ_type, n_stages, n_steps, ...
    N, Tf, dt_mpc, nx, nu, paramsFast, pert_scale, 10, ...
...
% =====================================================================
%  SHARED HELPER FUNCTIONS (identical across B and C)
% =====================================================================
...
function os = build_ocp(name, x_r, u_sym, xdot_r, f_expl_r, ...
    ...
    ocp.cost.W=blkdiag(Q,R); ocp.cost.W_0=blkdiag(Q,R); ocp.cost.W_e=Q*10;
    ...
    fprintf('OCP solver ready (SQP_RTI, %s, %d stages, %d steps).\n',integ_type,n_stages,n_steps);
end
...
function [PX,PU,Pstat,Psqp,Pqp,Pt,Perr,Ppos,Pang,Pslack,n_actual,diverged] = ...
    ...
    % Timing: 8 fields packed into rows of Pt
    Pt=zeros(8,sim_steps);
...
function print_summary(id, diverged, n_actual, sim_steps, Pstat, Pt, Psqp, Perr, usab, settling, roa_max)
    tv=Pt(1,1:n_actual); stv=Pstat(1:n_actual); sv=Psqp(1:n_actual);
    fprintf('\n--- %s Summary ---\n',id);
    fprintf('  Diverged:      %s\n',yn_(diverged));
