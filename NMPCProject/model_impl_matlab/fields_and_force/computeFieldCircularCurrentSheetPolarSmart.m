...
%% rho ~= 0 (2a)
tol = 1e-9;                  % very small offset
rho(abs(rho - r) < tol) = r + tol;
...
k2p = min(max(k2p,0),1-eps);
k2n = min(max(k2n,0),1-eps);

tol_ellipke = 1e-7;
[Kp,Ep] = ellipke(k2p,tol_ellipke);
[Kn,En] = ellipke(k2n,tol_ellipke);
...
% bz
alpha = (rho - r) ./ (rho + r);
alpha(abs(alpha) < tol) = 0;  % stabilize exactly at rho=r
