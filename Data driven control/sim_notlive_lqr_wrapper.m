%% Addition by data driven project:
...
%xeq = @(t) [0,0,zEq(1)+0.01*sin(t),zeros(1,9)]'; 

[t,x] = ode15s(@(t,x) f(x,-K*(x-xLp)-uLp), tSpan, x0); 

%[t,x] = ode15s(@(t,x) f(x,-K*(x-xeq(t))-uLp), tSpan, x0);
...
U = zeros(4,length(t));
% Compute "u" for VRFT using the LQR feedback matrix.
for i = 1:length(t)
    U(:,i) = -K * (x(i,:)'-xLp);
end
U = U';
...
%% The following writes to a file for use with D3C. Commented out by default.
d = array2table(x);
UL = array2table(U);
d = addvars(d, t, 'Before', 1, 'NewVariableNames', 'Time');
UL = addvars(UL, t, 'Before', 1, 'NewVariableNames', 'Time');
d.Properties.VariableNames(2:end) = ...
    {'x','y','z','alpha','beta','gamma','xdot','ydot','zdot','alphadot','betadot','gammadot'};
UL.Properties.VariableNames(2:end) = ...
    {'u1','u2','u3','u4'};

T = join(d,UL);
writetable(T, 'model_output.csv');
